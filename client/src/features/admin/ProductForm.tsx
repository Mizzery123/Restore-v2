...
type Props = {
  ...
  refetch: () => void //Takes no parameter and returns void
  setSelectedProduct: (value: Product | null) => void;
}

export default function ProductForm({ setEditMode, product, refetch, setSelectedProduct }: Props) {

  const { control, handleSubmit, watch, reset, setError, formState: { isSubmitting } } = useForm<CreateProductSchema>({ //isSubmitting for loading!
    mode: 'onTouched',
    resolver: zodResolver(createProductSchema)
  });

  const watchFile = watch('file'); //Create preview URL, stays in memory until manually cleared
  ...
  useEffect(() => {
    ...
    return () => {
      if (watchFile) URL.revokeObjectURL(watchFile?.preview) // Clean up url in memory
    }
  }, [product, reset, watchFile])

  // const onSubmit = (data: z.input<typeof createProductSchema>) => {
  //   // Parse it into CreateProductSchema
  //   const parsedData: CreateProductSchema = createProductSchema.parse(data);
  ...
  const createFormData = (items: FieldValues) => {
    ...
    return formData; //Rmb to return else the function returns VOID!
  }
  ...
  const onSubmit = async (data: CreateProductSchema) => {
    try {
      ...
      if (watchFile) formData.append('file', watchFile); //Get the file from watchfile(image upload) not the field values to send to API!
      ...
      console.log(error);
      handleApiError<CreateProductSchema>(error, setError, ['brand', 'description', 'file', 'name', 'pictureUrl',
